...
interface GeneratedKnowledgeNote {
  title: string
  slug: string
  domain: string
  topic: string
...
function buildKnowledge(): GeneratedKnowledgeNote[] {
  const notes: GeneratedKnowledgeNote[] = []
...
`
      ...
      console.log(
        `[routes] generated ${posts.length} posts, ${tags.length} tags, ${knowledge.length} knowledge notes -> ${GENERATED_FILE.replace(ROOT, '')}`,
      )
    },
  }
}
